...
BB4: MOV DL,CL ; 将 CX 的值（即个位数值）加载到 DL 寄存器
...
LOP2: CALL FANG ; 调用 FANG 子程序，计算当前循环计数的阶乘，并将结果存入 DX
...
LOP1: MUL CX ; 将 CX 寄存器的值乘以 AX 寄存器的值，结果存入 AX
DEC CX ; 循环计数减一
CMP CX,0 ; 检查循环计数是否为 0
